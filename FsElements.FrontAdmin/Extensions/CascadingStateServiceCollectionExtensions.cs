...
namespace FsElements.FrontAdmin.Extensions
{
    public static class CascadingStateServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddNotifyingCascadingValue<T>(
            this IServiceCollection services, Func<IServiceProvider, T> initialValueFactory, bool isFixed = false)
            where T : INotifyPropertyChanged
        {
            return services.AddCascadingValue<T>(sp =>
            {
                return new CascadingStateValueSource<T>(initialValueFactory(sp), isFixed);
            });
        }
